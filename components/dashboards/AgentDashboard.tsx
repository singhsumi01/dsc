  ...
  return (
    <div className="space-y-8 anim-fade-up">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
        <div>
          <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1.5">Agent Portal</p>
          ...
          <p className="text-gray-400 text-sm mt-1">Manage client applications and certificate issuance.</p>
        </div>
        <Link
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          { icon: Users, label: 'Total Clients', value: stats?.total ?? 0, iconBg: 'bg-blue-50', iconColor: 'text-blue-600' },
          { icon: Clock, label: 'Pending Verification', value: stats?.pending ?? 0, iconBg: 'bg-orange-50', iconColor: 'text-orange-500' },
          { icon: CheckCircle2, label: 'DSCs Issued', value: stats?.completed ?? 0, iconBg: 'bg-green-50', iconColor: 'text-green-600' },
          { icon: TrendingUp, label: 'Success Rate', value: '100%', iconBg: 'bg-purple-50', iconColor: 'text-purple-600' },
      ...
      <div className="bg-white border border-gray-100 rounded-2xl overflow-hidden">
        <div className="p-6 border-b border-gray-50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-gray-400 mt-0.5">{filtered.length} applications</p>
          </div>
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <input
                ...
                className="pl-9 pr-4 py-2 text-sm bg-gray-50 border border-gray-100 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-100 focus:border-blue-500 w-52"
              />
            </div>
            <Link href="/dashboard/applications" className="text-xs font-semibold text-blue-600 hover:underline">View All →</Link>
          </div>
        </div>

        {loading ? (
          <div className="p-6 space-y-3">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="skeleton h-14 w-full" />)}</div>
        ) : filtered.length === 0 ? (
          <div className="py-20 text-center">
            <FileText className="w-12 h-12 text-gray-200 mx-auto mb-3" />
            <p className="text-gray-400 font-medium text-sm">No applications found</p>
            <Link href="/dashboard/applications/new" className="text-sm text-blue-600 font-semibold mt-2 inline-block hover:underline">
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map(app => (
                  <tr key={app.ApplicationID} className="hover:bg-gray-50/60 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-gray-600 font-medium text-sm">{app.DSCType || 'Class 3'}</td>
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-bold uppercase tracking-wider ${STATUS[app.ApplicationStatus] || 'bg-gray-50 text-gray-600 border border-gray-100'}`}>
